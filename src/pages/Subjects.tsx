        ...
        {subjects.map((subject) => {
          ...
          return (
            <Link
              ...
              className="rounded-xl border border-mist-line bg-paper-raised p-4 hover:border-mist-line"
            >
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <h3 className="truncate text-sm font-semibold text-ink">{subject.name}</h3>
                  {subject.code && <p className="mt-0.5 truncate text-xs text-mist">{subject.code}</p>}
                </div>
                <button
                  type="button"
